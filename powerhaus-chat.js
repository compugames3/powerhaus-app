(function () {
    ...
    try {
      ...
      if (usersRes.ok) {
        ...
        const members = users.filter(u => {
          ...
          return !role.includes('admin') && !role.includes('entrenador') && !role.includes('trainer') && !role.includes('staff') && !name.includes('admin');
        });
        snapshot.members = members.length;
        snapshot.trainers = users.filter(u => String(u.role || '').toLowerCase().includes('entrenador')).length;
      }
    } catch (_) {}
  ...
  async function buildReply(text) {
    ...
    if (query.includes('qué puedes hacer') || query.includes('que puedes hacer') || query.includes('ayuda')) {
      return {
        text: 'Puedo ayudarte con tres cosas:\n• abrir secciones del sistema\n• darte contexto rápido de la página actual\n• mostrar datos básicos de miembros, clases o ventas cuando estén disponibles',
        links: [
    ...
    if (query.includes('resumen') || query.includes('panel')) {
      return {
        text: `Resumen rápido:\n• Página actual: ${currentConfig.title}\n• Miembros registrados: ${snapshot.members ?? 'sin datos'}\n• Entrenadores: ${snapshot.trainers ?? 'sin datos'}\n• Clases guardadas: ${snapshot.classes ?? 'sin datos'}\n• Ventas de hoy: ${snapshot.salesToday != null ? '$' + Number(snapshot.salesToday).toFixed(2) : 'sin datos'}`,
        links: [
    ...
    if (query.includes('miembro')) {
      return {
        text: `Ahora mismo tengo registrado este dato rápido:\n• Total de miembros: ${snapshot.members ?? 'sin datos'}\n\nSi quieres trabajar con miembros, puedo llevarte a la gestión o al alta de un nuevo perfil.`,
        links: [
    ...
    if (query.includes('entrenador')) {
      return {
        text: `Dato rápido:\n• Entrenadores registrados: ${snapshot.trainers ?? 'sin datos'}\n\nTambién puedo abrirte la sección para revisarlos o editar uno existente.`,
        links: [
          { label: 'Ver miembros y entrenadores', href: 'gestion de miembros.html?view=entrenadores' },
          { label: 'Crear usuario', href: 'crear_usuario.html' }
        ]
      };
    }

    if (query.includes('venta') || query.includes('cobro') || query.includes('pos') || query.includes('mes')) {
      return {
        text: `Ventas del día: ${snapshot.salesToday != null ? '
        links: [
    ...
    if (query.includes('clase')) {
      return {
        text: `Hay ${snapshot.classes ?? 'sin datos'} clases registradas en este momento. Aunque el bot no aparece en calendario, sí puedo llevarte a crear una clase nueva.`,
        links: [
    ...
    if (route) {
      return {
        text: `Encontré una coincidencia con ${route.label}. Si quieres, usa el acceso directo.` ,
        links: [{ label: `Abrir ${route.label}`, href: route.href }]
      };
    }

    return {
      text: `Estoy activo en ${currentConfig.title}. Puedes pedirme algo como:\n• “ir a miembros”\n• “resumen del panel”\n• “abrir ventas”\n• “qué puedes hacer aquí”`,
      links: []
    };
  }
 ...
 + Number(snapshot.salesToday).toFixed(2) : 'sin datos'}.\nVentas del mes: ${snapshot.salesMonth != null ? '
        links: [
          { label: 'Abrir punto de venta', href: 'punto de venta.html' },
          { label: 'Ver historial de ventas', href: 'historial_ventas.html' }
        ]
 ...
 + Number(snapshot.salesMonth).toFixed(2) : 'sin datos'}.\n\nSi quieres, abre el punto de venta o consulta el historial para revisar.`,
        links: [
          { label: 'Abrir punto de venta', href: 'punto de venta.html' },
          { label: 'Ver historial de ventas', href: 'historial_ventas.html' }
        ]
